...
DATA_DIR = Path("/tmp/mizenboshi") if os.getenv("VERCEL") else SOURCE_DATA_DIR
...
PROCESS_KNOWLEDGE_PATH = DATA_DIR / "mizenboshi_process.json"
...
FALLBACK_ANSWER = (
    "I couldn't find a confident answer in the Mizenboshi QA knowledge base. "
...
def root():
    return {
        "status": "ok",
        "message": "Mizenboshi QA Assistant API is running.",
...
def list_documents(principal: dict = Depends(require_permission("view"))):
    """Return the searchable Mizenboshi document library metadata."""
    return {
        "documents": [
            public_document(doc)
            for doc in DOCUMENTS
